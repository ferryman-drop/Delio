# TASK-006: Implement Context Funnel (Implementation Plan)

## 🚨 Проблема
Стан `RETRIEVE` не виконує свою функцію. Документація (`MEMORY_FUNNEL.md`) описує складну систему збору контексту (short-term, long-term, structured), але файл `core/memory/funnel.py` фізично відсутній або не реалізований. Бот працює "всліпу" або покладається на хаотичні шматки старого коду.

## 🛠️ Запропоновані Зміни
... (See below for implementation status)

## ✅ Статус Реалізації (Implementation Status) - Виконано ✅
1. **ContextFunnel (`core/memory/funnel.py`)**: Реалізовано клас, що агрегує дані з трьох джерел:
   - **Short-Term**: Redis (через `old_core.get_cached_context`).
   - **Long-Term**: ChromaDB (через `old_memory.search_memory`).
   - **Structured**: SQLite (через `memory_manager_v2`).
2. **RetrieveState (`states/retrieve.py`)**: Стан тепер викликає `funnel.aggregate_context` і зберігає результат у `context.memory_context`.
3. **Безпека**: Додано перевірку дозволів `Action.MEM_RETRIEVE` через `StateGuard`.
4. **Resiliency**: Використано `try/except` для кожного джерела даних, щоб збій одного не зупиняв роботу.

### Змінені/Створені файли:
- `core/memory/funnel.py`
- `states/retrieve.py`
